...
@Suite("WitnessItem Model Tests")
struct WitnessItemTests {
    
    @Test("WitnessItem initialization")
    func testInit() {
    ...
    @Test("WitnessItem hash hex")
    func testHashHex() {
        let hash = Data([0xAB, 0xCD, 0xEF])
        let item = WitnessItem(contentType: .text, contentHash: hash)
        
        #expect(item.hashHex == "abcdef")
    }
    
    @Test("WitnessItem display title - with title")
    ...
    @Test("WitnessItem display title - text truncation")
    ...
    @Test("WitnessItem status icon mapping")
    func testStatusIcons() {
        let item = WitnessItem(contentType: .text, contentHash: Data())
...
struct PDFExportServiceTests {
    ...
    func testGenerateCertificate() async throws {
        let snapshot = WitnessItemSnapshot(
            id: UUID(),
            createdAt: Date(),
            contentType: .text,
...
// Note: WitnessItemSnapshot memberwise init is defined in PDFExportService.swift
