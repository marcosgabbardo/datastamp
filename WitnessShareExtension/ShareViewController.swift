...
class ShareViewController: UIViewController {
    ...
    private func showError(_ message: String) {
        DispatchQueue.main.async { [weak self] in
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default) { _ in
                self?.extensionContext?.cancelRequest(withError: NSError(domain: "Witness", code: -1))
            })
    ...
    private func showSuccess() {
        ...
        updateStatus("✓ Timestamp created!\nOpen Witness app to see details.")
    ...
    @objc private func cancelTapped() {
        extensionContext?.cancelRequest(withError: NSError(domain: "Witness", code: 0))
    }
}
